...
<nav>
    <a href="?action=c&page=crea">Créer une fiche client</a>
    <a href="?action=c&page=mod">Modifier une fiche client</a>
</nav>
...
?>
        <form action="index.php?action=c&page=save" method="post">
    ...
    case 'save':
        ...
        $ins->execute([$_POST['nom'], $_POST['prenom'], $_POST['adresse'], $_POST['ville'], $_POST['cp'], $_POST['tel'], $_POST['mail'], $_POST['login'], password_hash($_POST['mdp'], PASSWORD_DEFAULT)]);
        echo ('Votre client a bien été enregistré avec succes!');
    ...
    case 'mod':
        ...
        $i = 0;
        echo '<form action="index.php?action=c&page=sup" method="post">';
        while ($r = $s->fetch()) {
            $lien = 'index.php?action=c&page=edit&id=' . $r['id'];
        ...
        echo '<button>Supprimé un client</button>';
        echo '</form>';
        break;
        break;
    ...
    case 'edit':
        ...
        $r = $s->fetch();
    ?>
        <form action="index.php?action=c&page=update&id=<?php echo $_GET['id']; ?>" method="post">
...
//		echo $req;
        ...
        $upd->execute();
        echo ('Votre client a bien été modifié avec succes!');
        break;
}
